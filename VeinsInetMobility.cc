...
void VeinsInetMobility::initialize(int stage)
{
    ...
    lonSignal = registerSignal("lon");
    latSignal = registerSignal("lat");
...
void VeinsInetMobility::nextPosition(const inet::Coord& position, std::string road_id, double speed, double angle)
{
    ...
    veins::Coord veins_coord = veins::Coord(lastPosition.x, lastPosition.y, lastPosition.z);

    //getCommandInterface()->getLonLat()

    std::cout << "Last Position: " << lastPosition.x << " | " << lastPosition.y << " | "  << lastPosition.z << endl;
    std::cout << getCommandInterface()->getLonLat(veins_coord).second << " | " << getCommandInterface()->getLonLat(veins_coord).first << endl;

    emit(lonSignal, getCommandInterface()->getLonLat(veins_coord).first);
    emit(latSignal, getCommandInterface()->getLonLat(veins_coord).second);
    emit(posZSignal, lastPosition.z);

    std::cout << "Success emiting signals" << endl;
